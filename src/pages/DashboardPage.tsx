      ...
      <div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm transition-all relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            ...
            >
              <h2 className="text-xl sm:text-2xl md:text-3xl font-black text-slate-800 dark:text-white leading-tight flex items-center gap-2 whitespace-nowrap">
                ...
                <ChevronDown size={20} className={cn("text-slate-400 transition-transform duration-300", showCalendar && "rotate-180")} />
              </h2>
          ...
          <div className="flex items-center bg-slate-50 dark:bg-slate-800/50 p-1.5 rounded-2xl border border-slate-100 dark:border-slate-800 w-full sm:w-auto justify-between sm:justify-start">
            <button onClick={prevMonth} className="p-2 md:p-3 hover:bg-white dark:hover:bg-slate-700 rounded-xl transition-all text-slate-600 dark:text-slate-300">
              <ChevronLeft size={18} />
            </button>
            <button onClick={() => setCurrentMonth(new Date())} className="px-4 py-2 text-sm font-bold text-slate-700 dark:text-slate-200 hover:bg-white dark:hover:bg-slate-700 rounded-xl transition-all">
              Mês Atual
            </button>
            <button onClick={nextMonth} className="p-2 md:p-3 hover:bg-white dark:hover:bg-slate-700 rounded-xl transition-all text-slate-600 dark:text-slate-300">
        ...
        {showCalendar && (
          <div className="mt-6 pt-6 border-t border-slate-100 dark:border-slate-800 animate-in fade-in slide-in-from-top-4 duration-300 relative z-10">
            <div className="grid grid-cols-7 gap-1 text-center mb-2">
              {weekDays.map(d => <div key={d} className="text-xs font-bold text-slate-400 py-2">{d}</div>)}
            </div>
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      !isCurrentMonth ? "text-slate-300 dark:text-slate-700" : "text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800",
                  ...
                  >
                    ...
                    {dayData && isCurrentMonth && (
                      <div className="flex gap-0.5 mt-1">
                        {dayData.ganhos > 0 && <div className="w-1 h-1 rounded-full bg-emerald-500" />}
                        {dayData.gastos > 0 && <div className="w-1 h-1 rounded-full bg-rose-500" />}
                      </div>
                    )}
                  </button>
                );
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
        <div className="bg-white dark:bg-slate-900 p-5 md:p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-xs font-bold text-slate-500 uppercase">Entradas</span>
          </div>
          <p className="text-2xl md:text-3xl font-black">R$ {totalGanhos.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
        </div>
        <div className="bg-white dark:bg-slate-900 p-5 md:p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-xs font-bold text-slate-500 uppercase">Saídas</span>
          </div>
          <p className="text-2xl md:text-3xl font-black">R$ {totalGastos.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
        </div>
        <div className={cn("p-5 md:p-6 rounded-3xl shadow-lg border transition-colors", saldo >= 0 ? "bg-primary text-white" : "bg-rose-600 text-white")}>
          <div className="flex items-center gap-3 mb-4">
            <div className="bg-white/20 p-2.5 rounded-xl"><Wallet size={20} /></div>
            <span className="text-xs font-bold uppercase">Saldo</span>
          </div>
          <p className="text-2xl md:text-3xl font-black">R$ {saldo.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
        </div>
      </div>
      <div>
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg md:text-xl font-bold">Movimentações</h3>
          <button onClick={handleGoToToday} className="flex items-center gap-2 text-sm font-bold text-primary bg-primary/10 px-4 py-2 rounded-xl">
        ...
        <div className="grid gap-4">
          {activeDays.map(dateStr => {
            ...
            return (
              <button key={dateStr} onClick={() => navigate(`/dia/${dateStr}`)} className="w-full flex items-center bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 hover:border-primary/50 transition-all text-left group">
                <div className="w-14 h-14 rounded-2xl flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-800 mr-4 group-hover:bg-primary/10 group-hover:text-primary transition-colors">
                  <span className="text-xs font-bold uppercase">{format(dateObj, 'EEE', { locale: ptBR })}</span>
                  <span className="text-xl font-black">{format(dateObj, 'dd')}</span>
                </div>
                <div className="flex-1">
                  <h4 className="font-bold">{format(dateObj, "dd 'de' MMMM", { locale: ptBR })}</h4>
                  <div className="flex gap-3 text-xs">
                    <span className="text-emerald-500">R$ {dayData.ganhos.toFixed(2)}</span>
                    <span className="text-rose-500">R$ {dayData.gastos.toFixed(2)}</span>
                  </div>
                </div>
                <div className="text-right flex items-center gap-4">
                  <div>
                    <p className="text-xs font-bold text-slate-400">SALDO</p>
                    <p className={cn("font-black", daySaldo < 0 && "text-rose-500")}>R$ {daySaldo.toFixed(2)}</p>
                  </div>
